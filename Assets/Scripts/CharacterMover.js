...
// Is the crouch button held down? We use this interface instead of checking
// for the crouch button directly so this script can also be used by AIs.
@System.NonSerialized
var inputCrouch : boolean = false;

// Is the sprint button held down? We use this interface instead of checking
...
class CharacterMotorMovement {
	...
	var pushPower : float = 4.0f;
	
	var maxForwardSpeed : float = 8.0;
	var maxSidewaysSpeed : float = 6.0;
	var maxBackwardsSpeed : float = 7.0;
	
	var generalSpeedModifier : float = 1;
	var sprintModifier : float = 1.75f;
	var crouchModifier : float = .35f;

	var maxSprintEnergy : float = 1f;
	var sprintEnergy : float = maxSprintEnergy;
	var sprintDrainRate : float = .35f;
	var sprintChargeRate : float = .15f;
...
function SetGeneralSpeed(x : float) {
	movement.generalSpeedModifier = x;
}
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	if (desiredMovementDirection == Vector3.zero)
		...
		var modifiedMaxForward : float = movement.maxForwardSpeed * movement.generalSpeedModifier;
		var modifiedMaxBackwards : float = movement.maxBackwardsSpeed * movement.generalSpeedModifier;
		var modifiedMaxSideways : float = movement.maxSidewaysSpeed * movement.generalSpeedModifier;

		if (grounded && inputSprint && !movement.isCrouching){
			DrainSprint();
			if (movement.sprintEnergy > 0){
		...
		var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? modifiedMaxForward : modifiedMaxBackwards) / modifiedMaxSideways;
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * modifiedMaxSideways;
		return length;
	}
}

function DrainSprint() {
	movement.sprintEnergy -= (Time.deltaTime*movement.sprintDrainRate)*movement.maxSprintEnergy;
	if (movement.sprintEnergy < 0){
		movement.sprintEnergy = 0;
	}
}

function RechargeSprint() {
	movement.sprintEnergy += (Time.deltaTime*movement.sprintChargeRate)*movement.maxSprintEnergy;
	if (movement.sprintEnergy > movement.maxSprintEnergy){
		movement.sprintEnergy = movement.maxSprintEnergy;
	}
}
